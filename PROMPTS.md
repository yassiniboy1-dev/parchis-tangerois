...
## Version « tout-en-un » (un seul texte à coller)

Construis-moi un jeu appelé « Mafia — مافيا » (type Loup-Garou) : un ou deux tueurs, un médecin, un détective, des civils. Jeu collectif à plusieurs téléphones, tout le monde dans la même pièce, chacun son téléphone, débats à voix haute. Pas de mode local, pas de téléphone central : chacun suit la partie en silence sur son écran.

TECHNIQUE — Un seul fichier index.html (vanilla JS + CSS, SDK Firebase compat et Google Fonts Marcellus + Manrope), plus manifest.json, sw.js et icônes pour la PWA. Multijoueur via Firebase Realtime Database (nœud parties/{CODE} avec jeu:'mafia', purge après 48 h), quotas gratuits respectés. Hébergement gratuit : GitHub Pages (branche gh-pages + script de rafraîchissement) et/ou Netlify (crédits mensuels de déploiement : regrouper les mises à jour). UI en français, mobile-first iPhone (safe-area, tap ≥ 36 px), jamais confirm/alert/prompt → modals custom, accents directs. Tests : moteur en fonctions pures testées en VM contre le vrai HTML ; simulation 5 clients VM + faux Firebase (changement de téléphone joueur ET hôte, forçage) passée 3× avant livraison ; revue adversariale après chaque gros changement réseau.

RÈGLES — 4 à 12 joueurs. Réglages hôte : 1 ou 2 tueurs (2 dès 6 joueurs), médecin/détective activables, révélation des morts (défaut oui), minuteur de débats (∞/2/5 min). L'hôte joue comme les autres, son téléphone arbitre sans lui montrer de secret. Code choisi par l'hôte (3-8 caractères) ou aléatoire. Carte de rôle visible seulement en maintenant le doigt ; deux tueurs se connaissent. Nuit simultanée à écrans identiques (civils = geste factice) ; soi-même interdit sauf médecin (toast, pas de case grisée) ; médecin jamais la même personne deux nuits de suite ; tueurs en désaccord → tirage déterministe. Aube : annonce publique + rapport privé à maintien du doigt chez tout le monde (détective : TUEUR/INNOCENT) ; les morts n'ont jamais d'info secrète. Jour : débats (compte à rebours synchronisé, alerte sans passage auto), vote secret pas pour soi, égalité → personne, dépouillement animé par cible. Victoire : village sans tueur ; tueurs ≥ autres vivants. Fin : rôles, déroulé nuit par nuit, intuitions des civils pour rire, confettis, rejouer.

ROBUSTESSE — Seul l'hôte écrit l'état (seq monotone) ; joueurs = feuilles à leur uid par manche+nuit ; sans place on n'écrit jamais. Écran d'attente « On attend : X » + badge 📴 (présence onDisconnect sans faux positifs) + bouton hôte « Continuer sans les absents » ; personne n'est supprimé sur déconnexion. « Reprendre ma place » partout, reprise du siège de l'hôte = transfert d'arbitrage. Reconnexion automatique si membre d'une partie active ; jamais deux écouteurs Firebase ; code réutilisé par un autre groupe jamais happé. PWA réseau-d'abord, version visible à l'accueil, réglages ⚙️ depuis accueil/salon/partie. Bots 🤖 ajoutés par l'hôte, joués par son téléphone selon les vraies règles, jamais proposés à la reprise.

LANGUES — Français/arabe par téléphone, RTL, test de parité des traductions.

AMBIANCE — Noir tangérois (fond quasi noir, or de lanterne #d9a93c, Marcellus doré, sélection blanc lumineux), avatars emoji. Visuels Nano Banana Pro (gemini-3-pro-image, clé Gemini à demander), style « 1950s film noir illustration of Tangier, deep indigo, near-black shadows, warm amber lantern light, no text » : fond 9:16 (ruelle de la médina la nuit, lanterne, brume, silhouette au borsalino), icône 1:1, cartes 3:2 tueur/médecin/détective/civils. Cinématiques plein écran par phase, vibrations. Sons ElevenLabs Sound Effects (clé à demander) normalisés : cloche funèbre (mort), carillon de médina (aube), glissando de harpe (sauvetage), bulletin dans l'urne (vote), oud triomphal (village), violoncelle sinistre (mafia), deux coups hésitants (égalité), carte retournée (rôles) ; vent Suno pour la nuit. Narrateur : récits FR/AR déterministes (2-3 variantes par événement, « Bien joué, docteur ! »), voix ElevenLabs Daniel (eleven_multilingual_v2, stability 0.55/similarity 0.75/style 0.35, mp3_22050_32), un fichier par variante et langue, versions sans prénom, lecture par <audio> playsinline (silencieux iOS, PWA), repli Web Audio puis synthèse ; bascule 🗣️ proposée à l'hôte au premier salon, conseil « un téléphone au milieu ».

MÉTHODE — Questions de cadrage une par une si besoin, construction par étapes livrées et testées, clés demandées seulement au moment utile et jamais écrites dans le code, CLAUDE.md tenu à jour.
