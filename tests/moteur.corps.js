...
// ---- mode rapide : les doubles font rejouer, le 3e est une faute ----
R.mode='rapide';
G.seats=['h','h','h','h'];
buildBoard(); /* markSelectable touche les nœuds du plateau */
const _setTimeout=setTimeout;
setTimeout=(f,ms)=>{ f(); return 0; };
reset(); G.cur=0; G.sixes=0;
G.pawns[0][0]={z:'t',sq:8};
G.desRestants=[4,4]; G.phase='anim'; tirageRapide();
TT('rapide : double compté et joué', G.sixes===1 && G.phase==='move');
G.desRestants.length=0; G.phase='anim'; endOfAction();
TT('rapide : relance après un double', G.phase==='roll' && G.cur===0);
G.desRestants=[2,5]; G.phase='anim'; tirageRapide();
TT('rapide : tirage simple remet le compteur', G.sixes===0 && G.phase==='move');
G.desRestants.length=0; G.phase='anim'; endOfAction();
TT('rapide : pas de relance sans double', G.cur===1);
reset(); G.cur=0; G.sixes=2;
G.pawns[0][0]={z:'t',sq:20}; G.lastMoved[0]=0;
G.desRestants=[3,3]; G.phase='anim'; tirageRapide();
TT('rapide : 3e double = dernier pion à la maison, tour perdu', G.pawns[0][0].z==='h' && G.cur===1);
reset(); G.cur=0; G.sixes=2;
G.pawns[0][0]={z:'c',s:3}; G.lastMoved[0]=0;
G.desRestants=[5,5]; G.phase='anim'; tirageRapide();
TT('rapide : corridor protégé au 3e double', G.pawns[0][0].z==='c' && G.pawns[0][0].s===3);
setTimeout=_setTimeout;
R.mode='classique'; G.seats=['h','ia','ia','ia']; G.sixes=0; G.cur=0; reset();
